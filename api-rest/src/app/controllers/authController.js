...
router.post('/authenticate', async (req, res) => {
    ...
    const user = await User.findOne({ email }).select('+password');// Dessa maneira msm o password estando com selec: false é possivel ser acessado
...
router.post('/forgot_password', async (req, res) =>{
    ...
    try{
        const user = await User.findOne({ email }); //função para procurar o email digitado no banco
...
router.post('/reset_password', async (req, res)=> {
    ...
    try{
        ...
        res.send(400).send({ error: 'Cannot reset password' });
